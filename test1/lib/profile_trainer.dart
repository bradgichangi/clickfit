...
class Profile_Trainer_Screen extends StatefulWidget{
  final String email;
  Profile_Trainer_Screen({Key? key, required this.email}) : super(key: key);


  @override
  Profile_Trainer createState() => Profile_Trainer(email: email);
}

class Profile_Trainer extends State<Profile_Trainer_Screen>{
  final String email;
  Profile_Trainer({Key? key, required this.email});
  ...
  Future<List<Post>> getPosts() async{
    ...
    await FirebaseFirestore.instance.collection("posts").where("id", isEqualTo: email).get().then((querySnapshot) {
      querySnapshot.docs.forEach((result) async{
        //   print("Posts:" + result.data().toString());
                            ...
                            Row(
                                ...
                                children: [
                                  ElevatedButton(
                                    ...
                                    onPressed: () {
                                      Navigator.push(context, MaterialPageRoute(builder: (context) => Edit_Profile_Trainer(email: email)));
                                    },
                                  ),
                                    ...
                                    itemBuilder: (BuildContext context, int index){
                                      ...
                                      return ListTile(
                                        leading: CircleAvatar(
                                          backgroundImage: Image.network(snapshot2.data[0].image_url).image,
                                        ),
